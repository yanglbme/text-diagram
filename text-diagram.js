  ...
  function _add_meta(in_ast) {
    ...
    function _traverse(ast) {
      ...
      if ("object_declaration" == ast.type) {
        ...
        var left_obj =
          meta.objs[Math.min(ast.meta.sender_index, ast.meta.receiver_index)];
        var right_obj =
          meta.objs[Math.max(ast.meta.sender_index, ast.meta.receiver_index)];
    ...
    for (var i = 0; i < meta.objs.length; ++i) {
      ...
      for (var j = 0; j < meta.notes[obj].length; ++j) {
        ...
        if ("left" == note_ast.attr.side) {
          meta.boxes[obj].x1 = Math.max(
            meta.boxes[obj].x1,
            pre_line_offset + 1 + note_width + 1 - half_box_width
          );
        }
      }
      ...
      if (i > 0) {
        ...
        for (var k = 0; k < meta.notes[pre_obj].length; ++k) {
          ...
          if ("right" == note_ast.attr.side) {
            meta.boxes[obj].x1 = Math.max(
              meta.boxes[obj].x1,
              meta.lines[pre_obj].x_offset +
                1 +
                _note_width(note_ast.attr.content)
            );
          }
        }
      }
        ...
        for (var k = 0; k < meta.messages[pre_obj].length; ++k) {
          ...
          if (msg_ast.meta.right_obj == obj) {
            meta.boxes[obj].x1 = Math.max(
              ...
              meta.lines[pre_obj].x_offset +
                1 +
                _msg_width(msg_ast.attr.message)
            );
          }
        }
      }
      ...
      if (i > 0) {
        ...
        for (var j = 0; j < meta.messages[pre_obj].length; ++j) {
          ...
          if (tmp_ast.meta.sender_index == tmp_ast.meta.receiver_index) {
            ...
            meta.boxes[obj].x1 = Math.max(
              meta.boxes[obj].x1,
              meta.lines[pre_obj].x_offset + 1 + message_width
            );
          }
        }
      }
      ...
      for (var j = 0; j < meta.notes[obj].length; ++j) {
        ...
        if ("right" == note.side) {
          meta.x_spans[obj].x2 = Math.max(
            meta.x_spans[obj].x2,
            meta.lines[obj].x_offset + 1 + note_width
          );
        }
      }
      for (var j = 0; j < meta.messages[obj].length; ++j) {
        ...
        if (tmp_ast.meta.sender_index == tmp_ast.meta.receiver_index) {
          ...
          meta.x_spans[obj].x2 = Math.max(
            meta.x_spans[obj].x2,
            meta.lines[obj].x_offset + 1 + message_width
          );
        }
      }
    }
  ...
  function _to_cimage(in_ast) {
    ...
    for (var i in meta.statements) {
      ...
      if ("message_statement" == ast.type) {
        ...
        var line_len =
          meta.lines[rightObj].x_offset - meta.lines[leftObj].x_offset - 1;

        var cmessage = _cmessage(
          ast.attr.message,
          line_len,
          s == leftObj,
          s == r
        ...
        _draw_cpoints(
          ccanvas,
          meta.lines[leftObj].x_offset + 1 - meta.min_x,
          ast.meta.y1,
          cmessage
        );
      } else if ("note_statement" == ast.type) {
        ...
        if ("right" == side) {
          _draw_cpoints(
            ccanvas,
            meta.lines[obj].x_offset + 1 - meta.min_x,
            ast.meta.y1,
            cnote
          );
        } else if ("left" == side) {
          _draw_cpoints(
            ccanvas,
            meta.lines[obj].x_offset - 1 - _note_width(content) - meta.min_x,
            ast.meta.y1,
            cnote
          );
        }
    ...
    if (isSelfMessage) {
      ...
      for (var idx = 0; idx < lines.length; idx++) {
        for (var i = 0; i < lines[idx].length; ++i) {
          cpoints.push(
            _cpoint(
              lines[idx].charAt(lines[idx].length - 1 - i),
              line_len - 1 - i - 1,
              1 + idx,
              0
            )
